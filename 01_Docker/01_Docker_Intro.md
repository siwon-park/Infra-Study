...
가상 머신은 `하이퍼바이저(Hypervisor)`라는 소프트웨어를 통해 물리적인 서버를 논리적으로 여러 개로 분할하는 것을 말한다.

각각의 가상 머신은 자체적인 운영체제(`Guest OS`)와 어플리케이션을 가지고 있기 때문에 가상 머신에서 실행되는 어플리케이션은 호스트의 운영체제(`Host OS`)와 독립적이다.

![image](https://github.com/siwon-park/Infra-Study/assets/93081720/854b9ac3-45b8-4a62-9cc7-2c7fc82606d6)

#### (1) 장점과 단점

##### 장점

- 호스트 운영체제와 완전히 분리되어 독립적으로 실행된다.
  - 각각의 가상 머신은 자체 운영 체제, 어플리케이션, 라이브러리 및 기타 의존성을 가진다.
- 다양한 운영체제에서 실행이 가능하다.
- 실행 중인 가상 머신의 스냅샷을 저장하여 나중에 재사용/공유 가능하다.
  - 단, 앱과 앱 실행에 필요한 것들 뿐만 아니라 컴퓨터 전체를 캡슐화 => GB 단위로 무거울 수밖에 없음

##### 단점

- 가상 머신은 무겁다.
  - 점점 많아지면 많아질수록 OS의 오버헤드가 발생할 수 밖에 없음
  - OS의 자원(메모리, CPU)과 저장 공간 등을 낭비
- 가상 머신을 설치하는 것과 설정하는 것은 복잡하다.
  - 매번 VM을 설치할 때마다 설정을 해줘야하며, 해당 설정을 직접 해야하기 때문에 100% 동일한 가상 환경이라고 볼 수 없음
  - 이는 개발 단계에서 제품 생산 단계로 어플리케이션을 배포하고자 할 때, 해당 프로젝트를 작업했던 VM 환경과 동일한 방식, 환경의 프로덕션 머신을 구성해야 한다는 단점을 유발함

<br>

### 2) 컨테이너(Container)

컨테이너란 어플리케이션과 라이브러리, 의존성을 함께 패키징하는 가상화 기술을 말한다. → 어플리케이션을 실행할 수 있는 환경

하나의 호스트 OS 위에 해당 OS가 기본적으로 내재하고 있거나 컨테이너 에뮬레이터를 지원하는 내장 컨테이너를 사용하여 `도커 엔진`이 컨테이너 기반의 가상 환경을 구축함.

도커는 호스트 OS의 커널을 공유한다.

![image](https://github.com/siwon-park/Infra-Study/assets/93081720/5a386e74-7bc2-40ec-b477-14888bdf5fef)
...
- 컨테이너는 VM과 달리 OS에 미치는 영향이 적고, 속도도 빠르고 용량이 작음
  - 가상 머신에 비해 가볍고 훨씬 많은 어플리케이션 실행 가능
- 배포 및 재구축이 쉬움
  - 컨테이너는 생성/시작/중지/삭제를 빠르게 수행 가능
- 앱과 앱의 실행에 필요한 것들만 캡슐화할 수 있음
  - 가상 머신에 비해 훨씬 더 작은 저장 공간
- 컨테이너의 최대 장점은 동일한 환경을 다른 사람과 공유 가능하다는 점

##### 단점

- 컨테이너의 생성 및 실행, 동작에 있어 일부 보안적인 취약점을 유발할 수 있음
- 하나의 호스트 OS와 커널을 기반으로 동작하기 때문에 컨테이너에서 실행되는 어플리케이션이 완벽하게 격리되지 않을 수도 있음
  - 또한 이는 공격자가 호스트 OS를 공격하여 다른 컨테이너들까지 공격할 수도 있다는 취약점 유발 

<br>

### 3) 도커의 가상화

도커의 가상화 수준은 `운영체제(OS) 수준의 가상화`이다.

컨테이너들을 보면 호스트 OS의 자원을 바탕으로 Docker Engine에 의해 동작하고, 각 컨테이너는 독립적인 파일 시스템을 가지고 있기 때문에 마치 호스트 OS 위에 개별 프로세스들이 떠있는 것처럼 보이기도 한다. (물론 실제 동작도 이와 유사하게 동작한다.)

이것 때문에 프로세스 수준의 가상화라고 착각하는 사람이 있는데, 절대 아님을 유의하자.

<br>

### 4) 컨테이너가 어플리케이션을 실행할 수 있는 이유

호스트 OS에 node.js가 설치되어 있지 않은데도 불구하고 컨테이너를 통해 node.js를 실행할 수 있는 이유는 무엇일까? node.js를 포함하고 있는 도커 이미지의 크기는 node.js 설치 파일보다 용량 크기가 작은데 어떻게 가능한 것인가?

그것은 바로 도커 컨테이너가 어플리케이션 실행에 필요한 (최소한의) 런타임과 종속성을 포함하고 있기 때문이다.
...
도커 컨테이너는 어플리케이션 실행에 필요한 `최소한의 런타임과 종속성만을 포함`해서 실행을 보장하고 이미지의 크기를 경량화 한다.

#### (2) 레이어드 이미지 아키텍처

도커 이미지는 여러 층(Layer)의 아키텍처로 구성되어 있는데, 각 레이어는 이전 레이어의 변경사항을 저장하고 새로운 내용을 추가하는 형태로 구성되어 있다.

또한 레이어들은 변경 사항이 있을 때만 변경된 부분을 저장하기 때문에 전체적으로 이미지 용량은 작을 수밖에 없다.

그리고 변화가 있을 때만 다시 작성하기 때문에 캐싱(caching)을 사용하여 이미지 빌드 속도가 빠르다.

#### (3) 멀티 스테이지 빌드

도커는 멀티 스테이지 빌드를 제공하는데, 이를 통해 각 빌드 단계에서의 임시 파일과 필요없는 종속성 파일을 제거하여 최종 이미지의 크기를 경량화 할 수 있는 것이다.
...
### 2) RUN, CMD, ENTRYPOINT 명령어 간 차이점
